...
const updateAccountBalanceLatest = `-- name: UpdateAccountBalanceLatest :one
UPDATE accounts
SET
    balance = balance + $1
WHERE
...
type UpdateAccountBalanceLatestParams struct {
	Amount float64 `json:"amount"`
	ID     int64   `json:"id"`
}

func (q *Queries) UpdateAccountBalanceLatest(ctx context.Context, arg UpdateAccountBalanceLatestParams) (Account, error) {
	row := q.db.QueryRowContext(ctx, updateAccountBalanceLatest, arg.Amount, arg.ID)
	var i Account
	err := row.Scan(
		&i.ID,
		&i.UserID,
