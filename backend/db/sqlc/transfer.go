...
func (s *Store) TransferTx(ctx context.Context, tr CreateTransferParams) (TransferTxResponse, error) {
	...
	err := s.execTx(ctx, func(q *Queries) error {
		...
		// update balance
		toArg := UpdateAccountBalanceLatestParams{
			Amount: tr.Amount,
			ID:     int64(tr.ToAccountID),
		}
		tx.ToAccount, errT = q.UpdateAccountBalanceLatest(context.Background(), toArg)
		if errT != nil {
			return errT
		}

		fromArg := UpdateAccountBalanceLatestParams{
			Amount: -1 * tr.Amount,
			ID:     int64(tr.FromAccountID),
		}
		tx.FromAccount, errT = q.UpdateAccountBalanceLatest(context.Background(), fromArg)
		if errT != nil {
			return errT
		}
